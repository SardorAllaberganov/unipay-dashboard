...
| `<DataTable>` per-column `ColumnMeta` (`headerClassName` / `cellClassName`) | ✅ | TanStack `react-table` module augmentation. Lets consumers collapse actions / status / amount columns to content via `meta: { headerClassName: 'w-[1%]' }` or right-align with `cellClassName: 'text-right'`. Applied retroactively to staff actions column (fixes the kebab drift) and used throughout Students tables. |
